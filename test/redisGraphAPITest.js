...
describe('RedisGraphAPI Test', () => {
    ...
    it('test null value to string', () => {
        return api.query("CREATE ( {nullValue:null} )").then(response => {
            return api.query("MATCH (n) RETURN n.nullValue").then(resultSet => {
                assert.ok(resultSet.hasNext())
                let record = resultSet.next();
